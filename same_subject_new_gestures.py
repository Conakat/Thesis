...
train_val_gestures = [1, 3, 0]  # first 3 gestures
test_gestures = [2, 4, 5]  # remaining 3 gestures

def split_by_gestures(x_dict, y_dict, selected_gestures):
    """
    x_dict: dictionary {gesture: samples}
    y_dict: dictionary {gesture: regression values}
...
# change subject and session every time for experimentation

x_train_split, y_train_reg_split = split_by_gestures(
    x_dict_train11,
    y_dict_train_reg11,
    train_val_gestures
)

x_val_split, y_val_reg_split = split_by_gestures(
    x_dict_val11,
    y_dict_val_reg11,
    train_val_gestures
)

x_test_split, y_test_reg_split = split_by_gestures(
    x_dict_test11,
    y_dict_test_reg11,
    test_gestures
)
...
train(model, optimizer, x_train_split, y_train_reg_split, train_val_gestures, x_val_split, y_val_reg_split, train_val_gestures, n_way, n_support, n_query, max_epoch, epoch_size, model_save_path, patience, min_delta, batch_size)
test_episode = 100
test(model, x_test_split, y_test_reg_split, test_gestures, n_way, n_support, n_query, test_episode, model_load_path.format(3), batch_size)
